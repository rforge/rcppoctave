<<setup, include=FALSE>>=
knit_engines$set(highlight = function(options) {
  f = tempfile()
  on.exit(unlink(f))
  writeLines(code <- options$code, f)
  #cmd = sprintf('highlight -S %s -O %s --no-doc %s', options$hi.syntax, options$hi.output, shQuote(f))
  cmd = sprintf('highlight %s --no-doc %s', options$engine.opts, shQuote(f))
  str(cmd)
  out = if (options$eval) system(cmd, intern = TRUE) else ''
  options$echo = FALSE # do not echo source code
...
<<kalman, engine='highlight', engine.opts='-e acid -S matlab -O latex', results='asis'>>=
...
## similarly for Gamma:
o_source(text="function [x] = orngg(n)
     x = randg(1, n);
end
")
## returns a 500 x 500 matrix
set.seed(42)
## same as 'lightblue' below so we have  'randg(a, N)/b' == rgamma(a,b)
plot(density(as.vector(matrix(.O$orngg(500),ncol=1))/2))

## about the same in R:
set.seed(42)
plot(density(rgamma(25000,1,1)), ylim=c(0,1.8))
lines(density(rgamma(25000,1,2)), col='gray')
lines(density(rgamma(25000,1,1/2)), col='lightblue')
@
