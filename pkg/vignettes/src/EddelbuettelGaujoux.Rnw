...
<<pace, engine='highlight', engine.opts='-e edit-emacs -S octave -O latex', results='asis'>>=
...
")
set.seed(42)
## 'flatten' 500x500 matrix into vector
## we have  'randg(a, N)/b' == rgamma(a,b)
x1 <- as.vector(matrix(.O$orngg(500), ncol=1))
x2 <- as.vector(matrix(.O$orngg(500), ncol=1))/2
x3 <- as.vector(matrix(.O$orngg(500), ncol=1))*2
plot(density(x1),
     ylim=c(0,1.8), main="Calling from Octave")
lines(density(x2), col='lightblue')
...
N <- 25000
plot(density(rgamma(N,1,1)),
     ylim=c(0,1.8), main="Calling rgamma from R")
lines(density(rgamma(N,1,2)), col='lightblue')
lines(density(rgamma(N,1,1/2)), col='gray')
@
}
