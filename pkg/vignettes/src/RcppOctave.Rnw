...
\proglang{R}.
It allows \proglang{Octave} functions to be called from an \proglang{R} session,
...
software\footnote{\url{http://www.mathworks.com}}, or to be/get -- at least -- a bit familiar with \proglang{Octave} \cite{Eaton2002}, which is free and open source, and is able to read and execute m-files, as long as they do not require Matlab-specific functions.
However, \proglang{R} users may have neither Matlab license, nor the
time/will to become \proglang{Octave}-skilled, and yet want to use algorithms written in
Matlab/\proglang{Octave} for their analyses and research.

Being able to run m-files or selectively use \proglang{Octave} functionalities
...
\proglang{Octave}, it is important to test the correctness of the port by
...
provide an interface between \proglang{R} and \proglang{Octave}, but appears
...
and facilitating the usage of \proglang{Octave/Matlab} code from \proglang{R}, by providing a lean interface that enables direct and easy interaction with an embedded \proglang{Octave} session.
...
\proglang{C++} framework defined by \citeCRANpkg{Rcpp}.

\section{Objectives \& Features}
The ultimate objective of \pkgname{RcppOctave} is to provide a two-way
interface between \proglang{R} and \proglang{Octave}, i.e. that allows calling
\proglang{Octave} from \proglang{R} and vice-versa.
...
Currently, the package focuses on accessing \proglang{Octave} functionalities
from \proglang{R} with:

\begin{itemize}
 \item An out-of-the-box-working embedded \proglang{Octave} session;
 \item Ability to run/source m-files from \proglang{R};
 \item Ability to evaluate \proglang{Octave} statements and function calls from
 \proglang{R};
 \item Ability to call \proglang{R} functions in \proglang{Octave}
 code\footnote{Currently only when run from \proglang{R} through
 \pkgname{RcppOctave}.};
 \item Transparent passage of variables between \proglang{R} and \proglang{Octave};
...
of the box embedded R session, typically via \citeCRANpkg{RInside}.

\section{Accessing Octave from R}

The \Rpkg{RcppOctave} defines the function \code{.CallOctave}, which acts as a
single entry point for calling \proglang{Octave} functions from \proglang{R}.
...
The function \code{.CallOctave} calls an \proglang{Octave} \underline{function}
...
cat(readLines(system.file('scripts/example.m', package=pkg)), sep="\n")
@
\end{Verbatim}
...
The \code{.O} object facilitates manipulating single \proglang{Octave}
variables, as it emulates an \proglang{R} environment-like object whose elements
would be the objects available in the current \proglang{Octave}
embedded session:
...
As illustrated above, \proglang{Octave} functions can be called through the
...
\proglang{Octave}, and alleviate calls to a set of commonly used
\proglang{Octave} functions.
...
Their names have been chosen to reflect the corresponding \proglang{Octave}
function, and, in some cases, aliases matching standard \proglang{R} names are
also provided, so that users not familiar with \proglang{Octave} can find their
...
# rename on the fly
o_get(c='a', d='b')
# o_get partially matches (here: 'aa' -> 'aaa')
o_get('aa')
# get a function
o_get('svd')
@
...
\proglang{Octave} session, so that the object they define are available, or the
...
\code{Rcpp::wrap}, for converting \proglang{R} types to \proglang{Octave}
...
library(bibtex)
write.bib(c('Rcpp', 'RcppOctave', 'RInside', 'knitr'), file='../Rpackages.bib')
@
