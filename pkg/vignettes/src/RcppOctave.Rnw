...
%cat("\\bibliography{", file.path(system.file(package=pkg), "REFERENCES"), "}\n", sep='')
%@
...
\title{\Rpkg{\Sexpr{pkg}}: \Sexpr{pkgtitle}}
\date{\Rpkg{\Sexpr{pkg}} -- Version \Sexpr{prettyVersion} [\Sexpr{prettyDate}]}
\begin{document}
\maketitle

The \Rpkg{RcppOctave} package provides a direct interface to Octave from R,
implemented using the \citeCRANpkg{Rcpp}.
It allows to call Octave functions in a similar way as calling
C/C++/Fortran functions using the R core function~\code{.Call}.
Since Octave uses a language that is mostly compatible with Matlab, the
\Rpkg{RcppOctave} may be used to run Matlab m-files.
