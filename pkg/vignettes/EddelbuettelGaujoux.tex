...
\abstract{
  ...
  access another numerical computing language from within R.  TODO: Expand
}
...
%% { \small
%% %% by hand:  %% highlight --enclose-pre --no-doc --out-format=latex --syntax=Matlab --style=edit-emacs
%% <<matrixEx, engine='highlight', engine.opts='-e edit-emacs -S octave -O latex', results='asis'>>=
%% e = mean(D)';
%% V = cov(D);
%% i = ones(k,1);
...
\hlfunctioncall{o_source}(text="
  function x = \hlfunctioncall{orngg}(n)
    x = \hlfunctioncall{randg}(1, n);
  end
")
\hlfunctioncall{set.seed}(42)
\hlcomment{## 'flatten' 500x500 matrix into vector}
\hlcomment{## we have  'randg(a, N)/b' == rgamma(a,b)}
x1 <- \hlfunctioncall{as.vector}(\hlfunctioncall{matrix}(.O$\hlfunctioncall{orngg}(500), ncol=1))
x2 <- \hlfunctioncall{as.vector}(\hlfunctioncall{matrix}(.O$\hlfunctioncall{orngg}(500), ncol=1))/2
x3 <- \hlfunctioncall{as.vector}(\hlfunctioncall{matrix}(.O$\hlfunctioncall{orngg}(500), ncol=1))*2
\hlfunctioncall{plot}(\hlfunctioncall{density}(x1),
     ylim=\hlfunctioncall{c}(0,1.8), main=\hlstring{"Calling from Octave"})
\hlfunctioncall{lines}(\hlfunctioncall{density}(x2), col=\hlstring{'lightblue'})
\hlfunctioncall{lines}(\hlfunctioncall{density}(x3), col=\hlstring{'gray'})
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{/home/edd/svn/rcppoctave/pkg/vignettes/figure/RJwrapper-rngGamma} 
...
N <- 25000
\hlfunctioncall{plot}(\hlfunctioncall{density}(\hlfunctioncall{rgamma}(N,1,1)),
     ylim=\hlfunctioncall{c}(0,1.8), main=\hlstring{"Calling rgamma from R"})
\hlfunctioncall{lines}(\hlfunctioncall{density}(\hlfunctioncall{rgamma}(N,1,2)), col=\hlstring{'lightblue'})
\hlfunctioncall{lines}(\hlfunctioncall{density}(\hlfunctioncall{rgamma}(N,1,1/2)), col=\hlstring{'gray'})
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{/home/edd/svn/rcppoctave/pkg/vignettes/figure/RJwrapper-rngGammaR} 
